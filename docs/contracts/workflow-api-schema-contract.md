...
| task.create                            | POST   | /api/workflows/task/create                               |
| task.update                            | POST   | /api/workflows/task/update                               |
| task.list                              | POST   | /api/workflows/task/list                                 |
| event.create                           | POST   | /api/workflows/event/create                              |
| event.update                           | POST   | /api/workflows/event/update                              |
| event.list                             | POST   | /api/workflows/event/list                                |
| event.listConflicts                    | POST   | /api/workflows/event/list-conflicts                      |
| project.create                         | POST   | /api/workflows/project/create                            |
| project.update                         | POST   | /api/workflows/project/update                            |
| project.setLifecycle                   | POST   | /api/workflows/project/set-lifecycle                     |
| project.list                           | POST   | /api/workflows/project/list                              |
| note.create                            | POST   | /api/workflows/note/create                               |
| note.update                            | POST   | /api/workflows/note/update                               |
| note.linkEntity                        | POST   | /api/workflows/note/link-entity                          |
| note.unlinkEntity                      | POST   | /api/workflows/note/unlink-entity                        |
| note.list                              | POST   | /api/workflows/note/list                                 |
| notification.list                      | POST   | /api/workflows/notification/list                         |
| notification.acknowledge               | POST   | /api/workflows/notification/acknowledge                  |
| notification.dismiss                   | POST   | /api/workflows/notification/dismiss                      |
| search.query                           | POST   | /api/workflows/search/query                              |
...
| capture.suggest                        | `entryId:string (required)`, `actor:ActorRef (required)`                                                                                                                                                      | `suggestedTitle:string (non-empty)`, `aiAssist:boolean`, `at:date-like`                                                                   | `Entry { ... }`                                                                                                                                                                                                         |
...
| task.create                            | `title:string (required, non-empty)`, `actor:ActorRef (required)`                                                                                                                                             | `taskId:string (non-empty)`, `description:string`, `scheduledFor:date-like`, `dueAt:date-like`, `projectId:string (non-empty)`, `sourceEntryId:string (non-empty)`, `at:date-like` | `Task { ... }`                                                                                                                                                                                                          |
| task.update                            | `taskId:string (required, non-empty)`, `actor:ActorRef (required)`, `at least one of [title, description, scheduledFor, dueAt, projectId]`                                                                 | `title:string`, `description:string or null`, `scheduledFor:date-like or null`, `dueAt:date-like or null`, `projectId:string (non-empty) or null`, `at:date-like` | `Task { ... }`                                                                                                                                                                                                          |
| task.list                              | `none`                                                                                                                                                                                                         | `status:one-of[planned, completed, deferred]`, `projectId:string (non-empty)`, `scheduledFrom:date-like`, `scheduledTo:date-like (>= scheduledFrom)` | `Array<TaskListItem { id:string, title:string, description?:string, status:string, scheduledFor?:string, dueAt?:string, projectId?:string, sourceEntryId?:string, completedAt?:string, deferredUntil?:string, createdAt:string, updatedAt:string }>` |
| event.create                           | `title:string (required, non-empty)`, `startAt:date-like (required)`, `actor:ActorRef (required)`                                                                                                            | `eventId:string (non-empty)`, `endAt:date-like`, `at:date-like`                                                                           | `Event { id:string, title:string, startAt:string, endAt?:string, syncState:string, createdAt:string, updatedAt:string }`                                                                                              |
| event.update                           | `eventId:string (required, non-empty)`, `actor:ActorRef (required)`, `at least one of [title, startAt, endAt]`                                                                                              | `title:string`, `startAt:date-like`, `endAt:date-like or null`, `at:date-like`                                                           | `Event { ... }`                                                                                                                                                                                                         |
| event.list                             | `none`                                                                                                                                                                                                         | `from:date-like`, `to:date-like (>= from)`, `syncState:one-of[local_only, pending_approval, synced]`, `sort:one-of[startAt_asc, startAt_desc, updatedAt_asc, updatedAt_desc]`, `limit:positive-integer` | `Array<Event { id:string, title:string, startAt:string, endAt?:string, syncState:string, createdAt:string, updatedAt:string }>`                                                                                      |
| event.listConflicts                    | `none`                                                                                                                                                                                                         | `eventId:string (non-empty)`                                                                                                               | `Array<EventConflict { eventId:string, conflictingEventId:string }>`                                                                                                                                                   |
| project.create                         | `name:string (required, non-empty)`, `actor:ActorRef (required)`                                                                                                                                              | `projectId:string (non-empty)`, `description:string`, `at:date-like`                                                                      | `Project { id:string, name:string, description?:string, lifecycle:string, createdAt:string, updatedAt:string }`                                                                                                        |
| project.update                         | `projectId:string (required, non-empty)`, `actor:ActorRef (required)`, `at least one of [name, description]`                                                                                                 | `name:string`, `description:string`, `at:date-like`                                                                                        | `Project { ... }`                                                                                                                                                                                                       |
| project.setLifecycle                   | `projectId:string (required, non-empty)`, `lifecycle:one-of[active, paused, completed] (required)`, `actor:ActorRef (required)`                                                                            | `at:date-like`                                                                                                                             | `Project { ... }`                                                                                                                                                                                                       |
| project.list                           | `none`                                                                                                                                                                                                         | `lifecycle:one-of[active, paused, completed]`                                                                                              | `Array<Project { id:string, name:string, description?:string, lifecycle:string, createdAt:string, updatedAt:string }>`                                                                                                |
| note.create                            | `body:string (required, non-empty)`, `actor:ActorRef (required)`                                                                                                                                              | `noteId:string (non-empty)`, `linkedEntityRefs:Array<non-empty-string>`, `at:date-like`                                                  | `Note { id:string, body:string, linkedEntityRefs:Array<string>, createdAt:string, updatedAt:string }`                                                                                                                  |
| note.update                            | `noteId:string (required, non-empty)`, `body:string (required, non-empty)`, `actor:ActorRef (required)`                                                                                                     | `at:date-like`                                                                                                                             | `Note { ... }`                                                                                                                                                                                                          |
| note.linkEntity                        | `noteId:string (required, non-empty)`, `entityRef:string (required, non-empty)`, `actor:ActorRef (required)`                                                                                                | `at:date-like`                                                                                                                             | `Note { ... }`                                                                                                                                                                                                          |
| note.unlinkEntity                      | `noteId:string (required, non-empty)`, `entityRef:string (required, non-empty)`, `actor:ActorRef (required)`                                                                                                | `at:date-like`                                                                                                                             | `Note { ... }`                                                                                                                                                                                                          |
| note.list                              | `none`                                                                                                                                                                                                         | `entityRef:string (non-empty)`                                                                                                             | `Array<Note { id:string, body:string, linkedEntityRefs:Array<string>, createdAt:string, updatedAt:string }>`                                                                                                           |
| notification.list                      | `none`                                                                                                                                                                                                         | `status:one-of[pending, sent, dismissed]`, `type:string (non-empty)`, `relatedEntity:{ entityType?:string (non-empty), entityId?:string (non-empty) }`, `limit:positive-integer` | `Array<Notification { id:string, type:string, message:string, status:string, relatedEntityType?:string, relatedEntityId?:string, createdAt:string, updatedAt:string }>`                                              |
| notification.acknowledge               | `notificationId:string (required, non-empty)`, `actor:ActorRef (required)`                                                                                                                                    | `at:date-like`                                                                                                                             | `Notification { ... }`                                                                                                                                                                                                  |
| notification.dismiss                   | `notificationId:string (required, non-empty)`, `actor:ActorRef (required)`                                                                                                                                    | `at:date-like`                                                                                                                             | `Notification { ... }`                                                                                                                                                                                                  |
| search.query                           | `query:string (required, non-empty)`                                                                                                                                                                           | `entityTypes:Array<non-empty-string>`, `limit:positive-integer`                                                                           | `Array<SearchResult { entityType:string, entityId:string, preview:string, updatedAt?:string }>`                                                                                                                       |
