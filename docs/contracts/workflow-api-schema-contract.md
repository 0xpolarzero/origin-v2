...
| approval.approveOutboundAction         | `actionType:one-of[event_sync, outbound_draft] (required)`, `entityType:string (required)`, `entityId:string (required, non-empty)`, `approved:boolean (required)`, `actor:ActorRef (required)`          | `at:date-like`                                                                                                                             | `{ approved:true, executed:true, executionId:string }`                                                                                                                                                                 |
...
| checkpoint        | `id:TEXT(optional)`, `name:TEXT(required)`, `snapshot_entity_refs:TEXT(required, default='[]')`, `snapshot_entities:TEXT(required, default='[]')`, `audit_cursor:INTEGER(required)`, `rollback_target:TEXT(required)`, `status:TEXT(required)`, `created_at:TEXT(required)`, `updated_at:TEXT(required)`, `recovered_at:TEXT(optional)` | `checkpoint_status_check_insert/update` enforce lifecycle; delete guard trigger protects notification related-entity references.                                                               |
